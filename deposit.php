  ...
  <div class="col-md-12">
    <div style="margin-left:30px;  margin-right:30px; margin-top:10px;" class="card" id="todaycard">
      <div class="card-header bg-light" >
        <h2 class="text-graident-lightcrimson" ><b><i class="fa fa-fw fa-money text-dark" ></i> <i>Add Deposit Report</i> </b></h2>
      </div>
      <div class="card-body">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header bg-dark" >
        <h3 class="modal-title text-white" id="exampleModalLabel"><b><i>Deposit</i></b></h3>
      ...
      <div class="modal-body">
        <form id="CountryNameForm"> 
          ...
          <div class="form-group row mb-2">
            <label for="inputPassword" class="col-lg-3 col-form-label">Deposit Amount:</label>
            <div class="col-lg-6">
              <input type="text" class="form-control"  name="deposit_amount" id="deposit_amount" placeholder="Deposit Amount">
            </div>
            <div class="col-lg-3">
...
<div class="modal fade" id="updexampleModal"  role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header bg-dark" >
        <h3 class="modal-title text-white" id="updexampleModalLabel"><b><i>Update Deposit</i></b></h3>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
      </div>
      <div class="modal-body">
        <form id="UpdCountryNameForm"> 
          ...
          <div class="form-group row mb-2">
            <label for="inputPassword" class="col-lg-3 col-form-label">Deposit Amount:</label>
            <div class="col-lg-6">
              <input type="text" class="form-control"  name="upddeposit_amount" id="upddeposit_amount" placeholder="Deposit Amount">
            </div>
            <div class="col-lg-3">
...
function Delete(ID){
  ...
  $.confirm({
    title: 'Delete!',
    content: 'Do you want to delete this deposit',
    type: 'red',
    typeAnimated: true,
    buttons: {
        tryAgain: {
...
function GetDataForUpdate(id){
  ...
  $.ajax({
        ...
        success: function (response) {  
            ...
            $('#depositID').val(id);
            $('#upddeposit_amount').val(dataRpt[0].deposit_amount);
...
function getSalaryReport(){
      ...
      $.ajax({
          ...
          success: function (response) {  
            ...
            for(var i=0; i<Rpt.length; i++){
              total += parseInt(Rpt[i].deposit_amount);
              finalTable = "<tr><th scope='row' class='text-center'>"+ j + "</th><td class='text-capitalize text-center'>"+
              Rpt[i].account_Name +"</td><td class='text-center'>"+Rpt[i].datetime+"</td><td class='text-center'>"+ numeral(Rpt[i].deposit_amount).format('0,0') + ' ' + Rpt[i].currencyName +"</td><td class='text-center'>"+Rpt[i].staff_name+"</td><td "+
              ...
              <?php } ?>
              <?php if($delete ==1) { ?>
                finalTable +="<button type='button'0 onclick='Delete("+ Rpt[i].deposit_ID +")'" +
    ...
    function AddSalary(){
      ...
      if(deposit_amount.val() == ""){
        notify('Validation Error!', 'Deposit amount is required', 'error');
        return;
      }
    ...
    function updateSalary(){
      ...
      var upddeposit_amount = $('#upddeposit_amount');
      if(upddeposit_amount.val() == ""){
        notify('Validation Error!', 'Deposit amount is required', 'error');
        return;
      }
      ...
      $.ajax({
        ...
        data: {
            UpdSalary:updSalary,
            Upddeposit_Amount:upddeposit_amount.val(),
