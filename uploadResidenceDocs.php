        ...
        for($i = 0; $i< $total; $i++ ){
            ...
            if(in_array($extension, $valid_extensions)){
                if($file_size <=10485760){
                    ...
                    $path = "residence/". $new_name;
                    move_uploaded_file($_FILES['file']['tmp_name'][$i],$path );
                    if(!$_FILES["file"]["error"][$i]){
                        $sql="INSERT INTO `residenceDocuments`(`ResID`, `file_name`, `original_name`) VALUES 
                        (:ResID,:file_name,:original_name)"; 
                        $stmt = $pdo->prepare($sql);
                        $stmt->bindParam(':ResID', $_POST['RID']);
