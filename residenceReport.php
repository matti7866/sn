  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-info" onclick="saveFinePaymentAndEmail()"><i class="fa fa-envelope me-1"></i> Save & Send Email</button>
        </form>
      </div>
    </div>
  </div>
...
<script>
  ...
  function setCircleAttributes(color) {
    $('.circle-progress-value').css({
      'stroke-width': '9px',
      'stroke': color,
      'stroke-linecap': 'round'
    });
    $('.circle-progress-circle').css({
      'stroke-width': '2px'
    });
    $('.circle-progress-text').css({
      'fill': 'white',
      'font-size': '18px',
      'font-weight': 'bold'
    });
  ...
  function saveFinePaymentAndEmail() {
    var INSERT_FINE_PAYMENT_EMAIL = "INSERT_FINE_PAYMENT_EMAIL";
    var Fine_payAmount = $('#Fine_payAmount');
    if (Fine_payAmount.val() == "" || Fine_payAmount.val() < 0) {
      notify('Validation Error!', 'payment amount is required', 'error');
      return;
    ...
    // Show loading indicator
    var saveEmailBtn = $('button[onclick="saveFinePaymentAndEmail()"]');
    var originalBtnText = saveEmailBtn.html();
    saveEmailBtn.html('<i class="fa fa-spinner fa-spin"></i> Processing...');
    saveEmailBtn.prop('disabled', true);
    ...
    $.ajax({
      ...
      url: "residenceReportController.php",
      data: {
        INSERT_FINE_PAYMENT_EMAIL: INSERT_FINE_PAYMENT_EMAIL,
        Fine_payAmount: Fine_payAmount.val(),
        Fine_remarks: fine_remarks.val(),
        Fine_account_id: fine_account_id.val(),
        ResFPaymentID: resFPaymentID.val(),
  ...
  // Helper function to refresh data after payment
  function refreshData() {
    Fine_payAmount = $('#Fine_payAmount');
    resFPaymentID = $('#resFPaymentID');
    ...
    // Refresh displays based on active tab
    if ($("#residenceTabs li a.active").attr('href') == "#default-tab-1") {
      getPendingResidence();
    } else if ($("#residenceTabs li a.active").attr('href') == "#default-tab-2") {
      getPendingPayForResidence();
