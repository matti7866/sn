<?php
    ...
    if($select == 0){
        echo "<script>window.location.href='pageNotFound.php'</script>";
                            ...
                            if($select == 1){
                            $fileExists = "SELECT file_name,original_name FROM residencedocuments WHERE ResidenceDocID = :ResidenceDocID";
                            $fileExistsStmt = $pdo->prepare($fileExists);
                            $fileExistsStmt->bindParam(':ResidenceDocID',$_GET['id']);
                            ...
                            if($fileInDatabase[0]['file_name']){
                                    ...
                                    //read file 
                                    readfile('residence/'. $fileInDatabase[0]['file_name']);
                                    exit;
                                }else{
