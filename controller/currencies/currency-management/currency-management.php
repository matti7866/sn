<?php
    ...
    if($select == 0 && $insert == 0 && $update == 0 && $delete == 0){
        ...
        echo json_encode(array('error' => 'All permission for currencies are denied for the user'));
        exit();
    }
    // populate account dropdown function
    if (isset($_GET['serverLabel']) && $_GET['serverLabel'] === 'Populate_Currency_Dropdown') {
        try{
            ...
            require_once '../../../api/currencies/currency-management/index.php';
            // create instance of customer
            $currency = new Currency($conn);
            // call getAccountDropdownOptions to get the list of currencies
            $currencies = $currency->getCurrencyDropdownOptions($q);
