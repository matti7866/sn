<?php
    ...
    if($select == 0){
        echo "<script>window.location.href='pageNotFound.php'</script>";
                            ...
                            if($select == 1){
                                $fileExists = "SELECT docName,originalName FROM residencefine WHERE residenceFineID = :residenceFineID";
                                $fileExistsStmt = $pdo->prepare($fileExists);
                                $fileExistsStmt->bindParam(':residenceFineID',$_GET['id']);
                                ...
                                if($fileInDatabase[0]['docName']){
                                    ...
                                    header("Content-Disposition: attachment; filename=".$fileInDatabase[0]['originalName']);
